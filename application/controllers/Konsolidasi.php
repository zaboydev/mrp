...
class Konsolidasi extends MY_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['konsolidasi'];
  ...
  public function index()
  {
    ...
    $this->data['page']['title']            = 'Laporan Konsolidasi GRN & MS';
  ...
  public function index_data_source()
  {
    ...
    if (is_granted($this->module, 'index') === FALSE){
      ...
      foreach ($entities as $row){
        ...
        $col[] = print_number($row['grn_usd']);
        $col[] = print_number($row['grn_rp']);
        $col[] = print_number($row['ms_usd']);
        $col[] = print_number($row['ms_rp']);
    ...
    if (is_granted($this->module, 'save') === FALSE){
      ...
      if ($this->input->post('id')){
          if ($this->model->update($this->input->post('id'))){
            $return['type'] = 'success';
            $return['info'] = 'Kurs Dollar Updated Rp.' . print_number($this->input->post('kurs_dollar'));
          } else {
            $return['type'] = 'danger';
            $return['info'] = 'There are error while updating data. Please try again later.';
          }
      } else {
        if ($this->model->isKursDateExists($this->input->post('tanggal'))){
          $return['type'] = 'danger';
          $return['info'] = 'Kurs Hari sudah Tersedia! Kurs Dollar hari ini adalah '.$this->model->today_kurs($this->input->post('tanggal'));
        } else {
          if ($this->model->insert()){
            $return['type'] = 'success';
            $return['info'] = 'Kurs Hari ini adalah ' . $this->input->post('kurs_dollar');
          } else {
            $return['type'] = 'danger';
            $return['info'] = 'There are error while updating data. Please try again later.';
          }
  ...
  public function delete()
  {
    ...
    if (is_granted($this->module, 'delete') === FALSE){
      ...
      if ($this->model->delete()){
        $return['type'] = 'success';
        $return['info'] = 'Kode ' . $this->input->post('kode_akunting') .' deleted.';
      } else {
        $return['type'] = 'danger';
