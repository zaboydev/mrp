...
class Departements extends MY_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['departements'];
  ...
  public function index()
  {
    ...
    $this->data['page']['title']            = 'Departements';
  ...
  public function index_data_source()
  {
    ...
    if (is_granted($this->module, 'index') === FALSE){
      ...
      foreach ($entities as $row){
        ...
        $col = array();
        $col[] = print_number($no);
        $col[] = print_string($row['division_name']);
        $col[] = print_string($row['department_name']);
        $col[] = print_string($row['department_code'], '');
  ...
  public function save()
  {
    ...
    if (is_granted($this->module, 'save') === FALSE){
      ...
      if ($this->input->post('id')){
        if ($this->model->isDepartmentNameExists($this->input->post('department_name'), $this->input->post('department_name_exception'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Department Name! Department '. $this->input->post('department_name') .' already exists.';
        } else {
          if ($this->model->update($this->input->post('id'))){
            $return['type'] = 'success';
            $return['info'] = 'Department ' . $this->input->post('department_name') .' updated.';
          } else {
            $return['type'] = 'danger';
        ...
        if ($this->model->isDepartmentNameExists($this->input->post('department_name'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Department! Department '. $this->input->post('department_name') .' already exists.';
        } else {
          if ($this->model->insert()){
            $return['type'] = 'success';
            $return['info'] = 'Department ' . $this->input->post('department_name') .' created.';
          } else {
            $return['type'] = 'danger';
            $return['info'] = 'There are error while updating data. Please try again later.';
          }
