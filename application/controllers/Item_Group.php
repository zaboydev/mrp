...
class Item_Group extends MY_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['group'];
  ...
  public function index()
  {
    ...
    $this->data['page']['title']        = 'Item Group';
      ...
      if ($this->input->post('id')){
        if ($this->model->isItemGroupExists($this->input->post('group'), NULL, $this->input->post('item_group_exception'))){
          ...
          if ($this->model->update($this->input->post('id'))){
            $return['type'] = 'success';
            $return['info'] = 'Item Group ' . $this->input->post('group') .' updated.';
          } else {
            $return['type'] = 'danger';
        ...
        if ($this->model->isItemGroupExists($this->input->post('group'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Item Group! Item Group '. $this->input->post('group') .' already exists.';
        } else {
          if ($this->model->insert()){
            $return['type'] = 'success';
            $return['info'] = 'Item Group ' . $this->input->post('group') .' created.';
          } else {
            $return['type'] = 'danger';
            $return['info'] = 'There are error while updating data. Please try again later.';
          }
