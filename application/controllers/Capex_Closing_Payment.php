...
class Capex_Closing_Payment extends MY_Controller
{
    protected $module;
    ...
    public function __construct()
    {
        ...
        $this->module = $this->modules['capex_closing_payment'];
        ...
        // if (empty($_SESSION['capex_closing']['request_to']))
        //   $_SESSION['capex_closing']['request_to'] = 1;
        // if (empty($_SESSION['capex_closing']['attachment']))
        //   $_SESSION['capex_closing']['attachment'] = array();
    }
    ...
    public function create($capex_request_id)
    {
        $this->authorized($this->module, 'document');
        $entity = $this->model->findRequestByid($capex_request_id);

        $_SESSION['capex_closing']['items']               = $entity['items'];
        $_SESSION['capex_closing']['document__number']    = $entity['pr_number'];
        $_SESSION['capex_closing']['date']                = $entity['required_date'];
        $_SESSION['capex_closing']['notes']               = $entity['notes'];
        $_SESSION['capex_closing']['closing_notes']       = NULL;
        $_SESSION['capex_closing']['account']             = NULL;
        $_SESSION['capex_closing']['id']                  = $capex_request_id;
    ...
    public function save()
    {
        ...
        if (is_granted($this->module, 'document') == FALSE){
            ...
            if (!isset($_SESSION['capex_closing']['account']) || empty($_SESSION['capex_closing']['account'])){
                $data['success'] = FALSE;
                $data['message'] = 'Account can not null! Please select account!';
            }else{
                if ($this->model->save()){
                    unset($_SESSION['capex_closing']);

                    $data['success'] = TRUE;
                    $data['message'] = 'Capex '. $document_number .' has been close to payment. You will redirected now.';
                } else {
                    $data['success'] = FALSE;
    ...
    public function set_date()
    {
        ...
        $_SESSION['capex_closing']['date'] = $_GET['data'];
    }
    ...
    public function set_notes()
    {
        ...
        $_SESSION['capex_closing']['closing_notes'] = $_GET['data'];
    }
    ...
    public function set_account()
    {
        ...
        $_SESSION['capex_closing']['account'] = $_GET['data'];
    }
