...
class Daftar_Akun extends MY_Controller
{
  protected $module;
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['daftar_akun'];
  ...
  public function save()
  {
    ...
    if (is_granted($this->module, 'save') === FALSE){
      ...
      if ($this->input->post('id')){
        if ($this->model->isCoaCodeExists($this->input->post('coa'), NULL, $this->input->post('coa_exception'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Account Code! Account Code '. $this->input->post('coa') .' already exists.';
        } else {
          if ($this->model->update($this->input->post('id'))){
            $return['type'] = 'success';
            $return['info'] = 'Account '.$this->input->post('coa').' - '.$this->input->post('group') .' has been set.';
          } else {
            $return['type'] = 'danger';
        ...
        if ($this->model->isCoaCodeExists($this->input->post('coa'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Account Code! Account Code '. $this->input->post('coa') .' already exists.';
        } else {
          if ($this->model->insert()){
            $return['type'] = 'success';
            $return['info'] = 'Account '.$this->input->post('coa').' - '.$this->input->post('group') .' created.';
          } else {
            $return['type'] = 'danger';
            $return['info'] = 'There are error while updating data. Please try again later.';
          }
  ...
  public function delete()
  {
    ...
    if (is_granted($this->module, 'delete') === FALSE){
      ...
      if ($this->model->delete()){
        $return['type'] = 'success';
        $return['info'] = 'Account '.$this->input->post('coa').' - '.$this->input->post('group') .' deleted.';
      } else {
        $return['type'] = 'danger';
