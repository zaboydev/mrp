...
class Adjustment extends MY_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['adjustment'];
  ...
  public function index_data_source()
  {
    ...
    foreach ($entities as $row){
      ...
      $col[] = print_date($row['date_of_entry'], 'd/m/Y');
  ...
  //   $this->data['page']['requirement']      = array('datatable');
  //   $this->data['grid']['column']           = array_values($this->model->getSelectedColumns());
  //   $this->data['grid']['data_source']      = site_url($this->module['route'] .'/index_data_source/'. $condition .'/'. $warehouse.'/'. $category.'/'.$quantity.'/'.$jenis.'/'.$start_date.'/'.$end_date);
  ...
  // public function index_data_source($condition = 'SERVICEABLE', $warehouse='ALL BASES', $category = 'all', $quantity = 'b', $jenis = 'stock', $start_date = NULL, $end_date = NULL)
  // {
  //   $this->authorized($this->module, 'index');
  ...
  //   $entities = $this->model->getIndex($condition, $warehouse,$quantity, $category, $jenis, $start_date, $end_date);
  ...
  //   $result = array(
  //     "draw" => $_POST['draw'],
  //     "recordsTotal" => $this->model->countIndex($condition, $warehouse,$quantity, $category, $jenis, $start_date, $end_date),
  ...
  public function create($category = NULL)
  {
    ...
    if ($category !== NULL){
      ...
      $_SESSION['adj']['items']            = array();
      $_SESSION['adj']['category']         = $category;
      $_SESSION['adj']['document_number']  = adj_last_number();
      $_SESSION['adj']['date']             = date('Y-m-d');
      $_SESSION['adj']['warehouse']        = config_item('auth_warehouse');
      $_SESSION['adj']['notes']            = NULL;

      redirect($this->module['route'] .'/create');
    }

    if (!isset($_SESSION['adj']))
  ...
  //   $_SESSION['adj']['items']            = array();
  //   $_SESSION['adj']['category']         = 'SPARE PART';
  //   $_SESSION['adj']['document_number']  = adj_last_number();
  //   $_SESSION['adj']['date']             = date('Y-m-d');
  //   $_SESSION['adj']['warehouse']        = config_item('auth_warehouse');
  //   $_SESSION['adj']['notes']            = NULL;
  ...
  public function search_stock_in_stores()
  {
    ...
    $category = $_SESSION['adj']['category'];
    ...
    foreach ($entities as $key => $value){
      ...
      $entities[$key]['label'] .= ' || ';
      $entities[$key]['label'] .= 'No GRN : '.$value['reference_document'].' || ';
      ...
      $entities[$key]['label'] .= 'On Hand Qty: <code>'. number_format($value['quantity']) .'</code>';
      $entities[$key]['label'] .= '</small>';
    }
  ...
  public function adj_add_item()
  {
    ...
    if (isset($_POST) && !empty($_POST)){
      $_SESSION['adj']['items'][] = array(
        ...
        'part_number'             => trim(strtoupper($this->input->post('part_number'))),
        'serial_number'           => trim(strtoupper($this->input->post('serial_number'))),
        'adj_quantity'            => $this->input->post('adj_quantity'),
        'adj_value'               => $this->input->post('adj_value'),
        'condition'               => $this->input->post('condition'),
        'stores'                  => trim(strtoupper($this->input->post('stores'))),
  ...
  public function set_warehouse()
  {
    ...
    $_SESSION['adj']['warehouse'] = $_GET['data'];
  }
