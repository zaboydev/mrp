...
class Item_Unit extends MY_Controller
{
  protected $module;
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['item_unit'];
  ...
  public function index()
  {
    ...
    $this->data['page']['title']            = 'Unit of Measurement';
  ...
  public function save()
  {
    ...
    if (is_granted($this->module, 'save') === FALSE){
      ...
      if ($this->input->post('id')){
        if ($this->model->isItemUnitExists($this->input->post('unit'), NULL, $this->input->post('unit_exception'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Unit! Unit '. $this->input->post('unit') .' already exists.';
        } else {
          if ($this->model->update($this->input->post('id'))){
            $return['type'] = 'success';
            $return['info'] = 'Unit ' . $this->input->post('unit') .' updated.';
          } else {
            $return['type'] = 'danger';
        ...
        if ($this->model->isItemUnitExists($this->input->post('unit'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Unit! Unit '. $this->input->post('unit') .' already exists.';
        } else {
          if ($this->model->insert()){
            $return['type'] = 'success';
            $return['info'] = 'Unit ' . $this->input->post('unit') .' created.';
          } else {
            $return['type'] = 'danger';
            $return['info'] = 'There are error while updating data. Please try again later.';
          }
