...
class Doc_Return extends MY_Controller
{
  protected $module;
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['doc_return'];
  ...
  public function save()
  {
    ...
    if (is_granted($this->module, 'save') == FALSE){
      ...
      if (!isset($_SESSION['return']['items']) || empty($_SESSION['return']['items'])){
        ...
        if (isset($_SESSION['return']['edit']) && $_SESSION['return']['edit'] !== $document_number){
          if ( $this->model->isDocumentNumberExists($document_number)){
            $data['success'] = FALSE;
            $data['message'] = 'Duplicate Document Number: '. $_SESSION['return']['document_number'] .' ! Try using '. return_last_number() .' for document number.';
          }
        }
  ...
  public function add_item($id = NULL)
  {
    ...
    if (isset($_POST) && !empty($_POST)){
      if (!isset($_SESSION['return'])){
        $_SESSION['return']['items']            = array();
        $_SESSION['return']['category']    		= available_categories($this->input->post('group'));
        $_SESSION['return']['document_number']  = return_last_number();
        $_SESSION['return']['issued_date']      = date('Y-m-d');
        $_SESSION['return']['issued_by']        = NULL;
        $_SESSION['return']['issued_to']        = NULL;
