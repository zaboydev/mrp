...
class Capex_Purchase_Order extends MY_Controller
{
  protected $module;
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['capex_purchase_order'];
    ...
    if (is_granted($this->module, 'index') === FALSE) {
      ...
      foreach ($entities as $row) {
        ...
        if (is_granted($this->module, 'approval')) {
          if ((config_item('auth_role') == 'HEAD OF SCHOOL') && ($row['review_status'] == strtoupper("waiting for hos review"))) {
  ...
  public function set_doc_number()
  {
    ...
    //   $_SESSION['order']['pom_document_number'] = $number;
    // }else{
    //   $_SESSION['order']['wom_document_number'] = $number;
    // }
  ...
  public function set_format_number()
  {
    ...
    // $_SESSION['order']['document_number'] = order_last_number($format_number);
  }
  ...
  public function info($id,$tipe)
  {
    ...
    if (is_granted($this->module, 'info') === FALSE) {
      ...
      if ($entity['status'] === 'evaluation') {
        $return['info'] = $this->load->view($this->modules['capex_order_evaluation']['view'] . '/info', $this->data, TRUE);
      } else {
  ...
  public function index_data_source_report()
  {
    ...
    if (is_granted($this->module, 'index') === FALSE) {
      ...
      foreach ($entities as $row) {
        ...
        $col[] = '<a href="' . site_url($this->modules['capex_order_evaluation']['route'] . '/print_pdf/' . $row['id']) . '" target="_blank" >' . print_string($row['poe_number']) . '</a><a href="#" class="btn btn-icon-toggle btn-info btn-sm "><i class="fa fa-eye" data-id="' . $row['poe_id'] . '"></i></a>';
  ...
  public function add_attachment_to_db($id)
  {
    ...
    $config['upload_path'] = 'attachment/capex_purchase_order/';
  ...
  public function cancel_ajax()
  {
    ...
    if (is_granted($this->module, 'document') === FALSE) {
      $alert['type']  = 'danger';
      $alert['info']  = 'You are not allowed to cancel this order!';
    } else {
      if ($this->model->cancel()) {
        $alert['type'] = 'success';
        $alert['info'] = 'PO canceled.';
        $alert['link'] = site_url($this->module['route']);
      } else {
        $alert['type'] = 'danger';
