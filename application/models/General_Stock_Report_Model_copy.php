...
class General_Stock_Report_Model_copy extends MY_Model
{
  ...
  public function getIndexSelectedColumns($tgl,$tgl_awal)
  {
    $return = array(
      ...
      null                                          => 'Initial Qty',
      'tb_master_item_groups.coa'                                          => 'Initial Value',
      'SUM(tb_receipt_items.received_quantity) AS grn_qty' => 'QTY GRN',
      ...
      'SUM(CASE WHEN tb_stock_cards.tgl <= '.$tgl.' THEN tb_stock_cards.quantity ELSE 0 END) AS qty'         => 'Balance',
      'SUM(CASE WHEN tb_stock_cards.tgl <= '.$tgl.' THEN tb_stock_cards.total_value ELSE 0 END) AS total_value'         => 'Balanca Value',
  ...
  public function getIndex($condition = 'SERVICEABLE', $warehouse= NULL, $start_date = NULL, $end_date = NULL, $category = NULL,  $return = 'array')
  {
    ...
    $this->db->join('tb_receipt_items', 'tb_stock_in_stores.id = tb_receipt_items.stock_in_stores_id');
    $this->db->join('tb_receipts', 'tb_receipts.document_number = tb_receipt_items.document_number');
    $this->db->join('tb_issuance_items', 'tb_stock_in_stores.id = tb_issuance_items.stock_in_stores_id');
    $this->db->join('tb_stock_adjustments', 'tb_stock_in_stores.id = tb_stock_adjustments.stock_in_stores_id');
  ...
  //       //$this->db->select('tb_stock_in_stores_reports.received_date');
  //       $this->db->where('tb_stock_in_stores_reports.received_date >= ', $range_issued_date[0]);
  //       $this->db->where('tb_stock_in_stores_reports.received_date <= ', $range_issued_date[1]);
  //   }if (empty($_POST['columns'][6]['search']['value'])){
  //     $this->db->select('tb_stock_in_stores_reports.received_date');
  //       $this->db->where('tb_stock_in_stores_reports.received_date ', $now);
  //   }
  ...
  //     $this->db->where('tb_master_item_categories.category', $search_category);
  //   }
  ...
  //   } else {
  //     $this->db->where('tb_stock_in_stores_reports.condition', 'SERVICEABLE');
  //   }
