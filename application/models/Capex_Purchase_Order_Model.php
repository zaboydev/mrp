...
class Capex_Purchase_Order_Model extends MY_Model
{
  ...
  {
    if (is_granted($this->data['modules']['capex_purchase_order'], 'approval') === TRUE) {
      return array(
        "''" . ' as "temp"'                 => "Act.",
        'tb_po.id'                          => 'No',
    ...
    if (!empty($_POST['columns'][4]['search']['value'])) {
      ...
      if ($status == 'review') {
        ...
        if (config_item('auth_role') == 'VP FINANCE') {
          $this->db->like('tb_po.review_status', 'WAITING FOR VP FINANCE REVIEW');
        }
        ...
        if (config_item('auth_role') == 'HEAD OF SCHOOL') {
          $this->db->where_in('tb_po.review_status', ['WAITING FOR COO REVIEW', 'WAITING FOR VP FINANCE REVIEW']);
        }
        if (config_item('auth_role') == 'VP FINANCE') {
          $this->db->where_in('tb_po.review_status', ['WAITING FOR HOS REVIEW', 'WAITING FOR COO REVIEW', 'APPROVED']);
        }
        if (config_item('auth_role') == 'CHIEF OPERATION OFFICER') {
  ...
  public function save_po()
  {
    ...
    foreach ($_SESSION['order']['items'] as $key => $item) {
      ...
      $this->db->set('unit', strtoupper($item['unit']));
      ...
      $prl_item_id = getPrlid($item['purchase_order_evaluation_items_vendors_id']);
      $this->connection->set('capex_purchase_requisition_detail_id',$prl_item_id);
      $this->connection->set('poe_qty',0);
      $this->connection->set('poe_value',0);
      $this->connection->set('po_qty',$item['quantity']);
      $this->connection->set('po_value',$item['total_amount']);
  ...
  public function save_revisi_po()
  {
    ...
    foreach ($result as $data) {
      ...
      //update di capex item progress db budgetcontrol
      $prl_item_id = getPrlid($data['poe_item_id']);
      $this->connection->set('capex_purchase_requisition_detail_id',$prl_item_id);
      $this->connection->set('poe_qty',0);
      $this->connection->set('poe_value',0);
      $this->connection->set('po_qty',floatval(0-$data['quantity']));
      $this->connection->set('po_value',floatval(0-$data['total_amount']));
      $this->connection->set('grn_qty',0);
      $this->connection->set('grn_value',0);
      $this->connection->insert('tb_capex_purchase_requisition_detail_progress');
      //end
  ...
  public function send_mail($doc_id, $level)
  {
    ...
    $message .= "<p>Berikut permintaan Persetujuan untuk Capex Purchase Order :</p>";
    $message .= "<ul>";
    $message .= "</ul>";
    ...
    $this->email->subject('Permintaan Approval Capex Purchase Order No : ' . $row['document_number']);
