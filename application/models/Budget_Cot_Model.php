...
class Budget_Cot_Model extends MY_Model
{
  ...
  public function itemCot()
  {
    ...
    $this->db->select('tb_master_part_number.id,tb_master_part_number.description,tb_master_part_number.part_number,tb_master_part_number.unit');
    ...
    // $this->db->where('tb_auth_user_categories.username', config_item('auth_username'));
    $this->db->group_start();
    $this->db->like('tb_master_part_number.description', trim(strtoupper($this->input->post('search'))), 'BOTH');
    $this->db->or_like('tb_master_part_number.part_number', trim(strtoupper($this->input->post('search'))), 'BOTH');
    // $this->db->or_like('tb_master_items.serial_number', trim(strtoupper($this->input->post('search'))), 'BOTH');
    $this->db->group_end();
    $this->db->group_by('tb_master_part_number.id,tb_master_part_number.description,tb_master_part_number.part_number,tb_master_part_number.unit');
    // if (($_POST['id_kategori'] != "all") && ($this->input->post('id_kategori') != ""))
    //   $this->db->where('tb_master_item_categories.category', $this->input->post('id_kategori'));
    if ($_POST['length'] != -1)
      $this->db->limit($_POST['length'], $_POST['start']);
  ...
  public function countItemCot()
  {
    ...
    // $this->db->where('tb_auth_user_categories.username', config_item('auth_username'));
    if (($_POST['id_kategori'] != "all") && ($this->input->post('id_kategori') != ""))
      $this->db->where('tb_master_part_number.category', $this->input->post('id_kategori'));
    $this->db->group_start();
    $this->db->like('tb_master_part_number.description', trim(strtoupper($this->input->post('search'))), 'BOTH');
    $this->db->or_like('tb_master_part_number.part_number', trim(strtoupper($this->input->post('search'))), 'BOTH');
    $this->db->group_end();
  ...
  public function cotProcess($hour, $year, $id_kelipatan, $kelipatan, $itemKey, $standardQuantity, $range1, $range2)
  {
    ...
    foreach ($itemKey as $key) {
      ...
      $exist = $this->checkCotItems($hour, $year, $id_kelipatan, $item_id, $standardQuantity->$key);
      $onhand = $this->countOnhand($part_number)->sum;
      if ($exist['status']) {
        $id = $exist['id'];
        $qty_standar = $standardQuantity->$key; //$standardQuantity->$key
        $oldData = $exist['data'];
        $range1_val  = $range1->$key;
        $range2_val  = $range2->$key;

        $update = $this->updateCot($id, $hour, $year, $id_kelipatan, $kelipatan, $item_id, $qty_standar, $onhand, $oldData);
        if ($update['status']) {
          $result['update'] += 1;
          $this->cotToBudgeting($id, $update['qty_requirement'], $hour, $item_id, $onhand, $range1_val, $range2_val);
        } else {
          $item = $this->itemById($item_id);
        ...
        $insert = $this->insertCot($hour, $year, $id_kelipatan, $kelipatan, $item_id, $qty_standar, $onhand, $part_number, $oldData);

        if ($insert['status']) {
          $result['insert'] += 1;
          $id_cot = $this->db->insert_id();
          // $this->cotToBudgeting($id_cot,$insert['qty_requirement'],$hour,$key,$onhand);
          $this->cotToBudgeting($id_cot, $insert['qty_requirement'], $hour, $item_id, $onhand, $range1_val, $range2_val);
        } else {
          $item = $this->itemById($item_id);
          array_push($result['error'], "Item " . $item->description);
        }
  ...
  function checkCotItems($hour, $year, $id_kelipatan, $id_item,$std_qty)
  {
    $oldData['data'] = null;
    if ($id_kelipatan === "0") {
    ...
    if ($id_kelipatan === "9") {
      ...
      $oldData['data'] = new \stdClass;
      $oldData['data']->qty_standar = "1";
      $oldData['data']->qty_requirement = $std_qty;
      $oldData['data']->hours = $hour;
      $oldData['data']->id_kelipatan = 9;
    }
