...
class Doc_Return_Model extends MY_Model
{
  protected $module;
  ...
  public function __construct()
  {
    ...
    $this->module = config_item('doc_return_table');
  }

  public function getSelectedColumns()
  {
    return array(
      'tb_doc_returns.id'               => NULL,
      'tb_doc_returns.document_number'  => 'Document Number',
      'tb_doc_returns.warehouse'        => 'Origin',
      'tb_doc_returns.issued_to'        => 'Destination',
      'tb_doc_returns.issued_date'      => 'Sent Date',
      'tb_doc_returns.issued_by'        => 'Issued/Released By',
      'tb_doc_returns.sent_by'          => 'Sent/Packed By',
      'tb_doc_returns.category'         => 'Category',
      'tb_doc_returns.notes'            => 'notes',
    );
  }
  ...
  {
    return array(
      'tb_doc_returns.document_number',
      'tb_doc_returns.warehouse',
      'tb_doc_returns.issued_to',
      'tb_doc_returns.issued_by',
      'tb_doc_returns.sent_by',
      'tb_doc_returns.category',
      'tb_doc_returns.notes',
    );
  }
  ...
  private function searchIndex()
  {
    if (!empty($_POST['columns'][2]['search']['value'])){
      ...
      $this->db->where('tb_doc_returns.issued_date >= ', $range_issued_date[0]);
      $this->db->where('tb_doc_returns.issued_date <= ', $range_issued_date[1]);
    }
    ...
    if (!empty($_POST['columns'][11]['search']['value'])){
      ...
      $this->db->where('tb_doc_returns.warehouse ', $search_warehouse);
    }

    if (!empty($_POST['columns'][12]['search']['value'])){
      $search_issued_to = $_POST['columns'][12]['search']['value'];
      $this->db->where('tb_doc_returns.issued_to ', $search_issued_to);
    }
  ...
  function getIndex($return = 'array')
  {
    ...
    $this->db->from('tb_doc_returns');
    $this->db->where_in('tb_doc_returns.category', config_item('auth_inventory'));
    $this->db->where_in('tb_doc_returns.warehouse', config_item('auth_warehouses'));
    $this->db->or_where_in('tb_doc_returns.issued_to', config_item('auth_warehouses'));
  ...
  public function findById($id)
  {
    ...
    $this->db->where('tb_doc_returns.id', $id);
    $this->db->from('tb_doc_returns');
    $this->db->join('tb_master_warehouses origin', 'origin.warehouse = tb_doc_returns.warehouse');
    $this->db->join('tb_master_vendors destination', 'destination.vendor = tb_doc_returns.issued_to');
    $query = $this->db->get();
    ...
    $this->db->from('tb_doc_return_items');
    $this->db->join('tb_master_items', 'tb_master_items.id = tb_doc_return_items.item_id');
    $this->db->where('tb_doc_return_items.document_number', $row['document_number']);
    $query = $this->db->get();
  ...
  public function isDocumentNumberExists($document_number)
  {
    ...
    $query = $this->db->get('tb_doc_returns');
  ...
  public function save()
  {
    ...
    $issued_to        = $_SESSION['return']['issued_to'];
    $issued_by        = $_SESSION['return']['issued_by'];
    $sent_by          = $_SESSION['return']['sent_by'];
    $approved_by      = $_SESSION['return']['approved_by'];
    $category         = $_SESSION['return']['category'];
    $warehouse        = $_SESSION['return']['warehouse'];
    $notes            = $_SESSION['return']['notes'];
    ...
    if ($document_id === NULL){
      ...
      $this->db->where('document_number', strtoupper($document_edit));
      $this->db->delete('tb_doc_return_items');
    ...
    foreach ($_SESSION['return']['items'] as $key => $data){
      ...
      $this->db->insert('tb_doc_return_items');
