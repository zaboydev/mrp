...
class Business_Trip_Request_Model extends MY_Model
{
    ...
    {
        $return = array(
            'tb_spd_advance_payments.id'                                                 => NULL,
            'tb_spd_advance_payments.document_number as no_transaksi'                    => 'Transaction Number',
            'tb_spd_advance_payments.tanggal'                                            => 'Date',
            'tb_spd_advance_payments.no_cheque'                                          => 'No Cheque',
            'tb_spd_advance_payments.vendor'                                             => 'Pay TO',
            'tb_spd_advance_payments.currency'                                           => 'Currency',
            'tb_spd_advance_payments.coa_kredit'                                         => 'Account',
            'SUM(tb_spd_advance_payments_details.amount_paid) as amount_paid'             => 'Amount IDR',
            'tb_spd_advance_payments.akun_kredit'                                        => 'Amount USD',
            'tb_spd_advance_payments.status'                                             => 'Status',
            'tb_spd_advance_payments.rejected_notes'                                     => 'Attachment',
            'tb_spd_advance_payments.base'                                               => 'Base',
            'tb_spd_advance_payments.notes'                                              => 'Notes',
        );
        if(is_granted($this->data['modules']['expense_closing_payment'], 'approval')){
            $return['tb_spd_advance_payments.approval_notes']  = 'Input Notes';
        }else{
            $return['tb_spd_advance_payments.approval_notes']  = 'Approval/Rejected Notes';
        }
    ...
    public function getSearchableColumnsForPayment()
    {
        $return = array(
            ...
            'tb_spd_advance_payments.document_number',
            // 'tb_purchase_order_items_payments.tanggal',
            'tb_spd_advance_payments.no_cheque',
            // 'tb_spd_advance_payments.document_number',
            // 'tb_po_item.part_number',
            // 'tb_purchase_order_items_payments.deskripsi',
            'tb_spd_advance_payments.currency',
            'tb_spd_advance_payments.coa_kredit',
            'tb_spd_advance_payments.akun_kredit',
            // 'tb_purchase_order_items_payments.amount_paid',
            'tb_spd_advance_payments.created_by',
            'tb_spd_advance_payments.vendor',
            'tb_spd_advance_payments.status',
            'tb_spd_advance_payments.base'
    ...
    public function getOrderableColumnsForPayment()
    {
        $return = array(
            NULL,
            'tb_spd_advance_payments.document_number',
            'tb_spd_advance_payments.tanggal',
            'tb_spd_advance_payments.no_cheque',
            ...
            // 'tb_purchase_order_items_payments.amount_paid',
            'tb_spd_advance_payments.base',
            'tb_spd_advance_payments.notes',
            // 'tb_spd_advance_payments.created_at'
        );
    ...
    public function getGroupedColumnsForPayment()
    {
        $return = array(
            ...
            'tb_spd_advance_payments.base',
            'tb_spd_advance_payments.notes',
            'tb_spd_advance_payments.approval_notes',
            'tb_spd_advance_payments.rejected_notes'
        );
    ...
    private function searchIndexForPayment()
    {
        if (!empty($_POST['columns'][1]['search']['value'])) {
            ...
            $this->db->where('tb_spd_advance_payments.tanggal >= ', $range_received_date[0]);
            $this->db->where('tb_spd_advance_payments.tanggal <= ', $range_received_date[1]);
        }

        if (!empty($_POST['columns'][2]['search']['value'])) {
            $vendor = $_POST['columns'][2]['search']['value'];

            $this->db->where('tb_spd_advance_payments.vendor', $vendor);
        }
        ...
        if (!empty($_POST['columns'][3]['search']['value'])) {
            ...
            if ($currency != 'all') {
                $this->db->where('tb_spd_advance_payments.currency', $currency);
            }
        }
        ...
        if (!empty($_POST['columns'][4]['search']['value'])) {
            ...
            if($status!='all'){
                $this->db->like('tb_spd_advance_payments.status', $status);
            }           
        } else {
            if(is_granted($this->data['modules']['expense_closing_payment'], 'approval')){
                ...
                $this->db->where_in('tb_spd_advance_payments.status', $status);
        ...
        if (!empty($_POST['columns'][5]['search']['value'])) {
            ...
            if($base!='ALL'){
                if($base!='JAKARTA'){
                    $this->db->where('tb_spd_advance_payments.base !=','JAKARTA');
                }elseif($base=='JAKARTA'){
                    $this->db->where('tb_spd_advance_payments.base','JAKARTA');
                }   
            }
        ...
        if (!empty($_POST['columns'][6]['search']['value'])) {
			...
			if($type!='all'){
				$this->db->like('tb_spd_advance_payments.type', $type);
			}			
		}
		...
		if (!empty($_POST['columns'][7]['search']['value'])) {
			...
			if($account!='all'){
				$this->db->like('tb_spd_advance_payments.coa_kredit', $account);
			}			
		}
    ...
    function getIndexForPayment($return = 'array')
    {
        ...
        $this->db->from('tb_spd_advance_payments');
        $this->db->join('tb_spd_advance_payments_details', 'tb_spd_advance_payments.id = tb_spd_advance_payments_details.spd_payment_id');
        $this->db->where('tb_spd_advance_payments.source','SPD');
        ...
        if (isset($_POST['order'])) {
            ...
            $this->db->order_by('tb_spd_advance_payments.tanggal', 'desc');
        }
    ...
    public function savePayment()
    {
        ...
        if (isset($_SESSION['spd_payment']['id'])) {
            ...
            $this->db->update('tb_spd_advance_payments');
        ...
        $this->db->insert('tb_spd_advance_payments');
        $spd_payment_id = $this->db->insert_id();
        ...
        $this->db->set('document_type','SPD PAYMENT');
        $this->db->set('document_number',$document_number);
        $this->db->set('document_id', $spd_payment_id);
        ...
        foreach ($spd_ids as $key=>$spd_id){           
            ...
            $this->db->set('spd_payment_id', $spd_payment_id); 
            ...
            $this->db->insert('tb_spd_advance_payments_details');
    ...
    {
        $this->db->select('tb_spd_advance_payments.*');
        $this->db->where('tb_spd_advance_payments.id', $id);
        $this->db->from('tb_spd_advance_payments');
        $query    = $this->db->get();
        $request  = $query->unbuffered_row('array');

        $select = array(
            'tb_spd_advance_payments_details.id',
            'tb_spd_advance_payments_details.spd_id',
            'tb_spd_advance_payments_details.amount_paid',
            ...
            'tb_business_trip_purposes.duration',
            'tb_spd_advance_payments_details.account_code',
            'tb_master_business_trip_destinations.business_trip_destination',
            'tb_spd_advance_payments_details.remarks'
        );

        $this->db->select($select);
        $this->db->from('tb_spd_advance_payments_details');
        $this->db->join('tb_business_trip_purposes', 'tb_business_trip_purposes.id = tb_spd_advance_payments_details.spd_id');
        $this->db->join('tb_business_trip_purpose_items', 'tb_business_trip_purpose_items.business_trip_purpose_id = tb_business_trip_purposes.id');
        $this->db->join('tb_master_business_trip_destinations', 'tb_master_business_trip_destinations.id = tb_business_trip_purposes.business_trip_destination_id');
        $this->db->where('tb_spd_advance_payments_details.spd_payment_id', $id);
    ...
    function save_pembayaran()
    {
        ...
        foreach ($_SESSION['bayar']['request'] as $i => $request) {
            $this->db->set('payment_status', "PAID");      
            $this->db->set('paid_amount', '"paid_amount" + ' . $request["amount_paid"], false);
            $this->db->set('paid_at', date("Y-m-d",strtotime($tanggal)));         
            $this->db->where('id', $request['spd_id']);
            $this->db->update('tb_business_trip_purposes');            

            if ($currency == 'IDR') {
                $amount_idr = $request["amount_paid"];
                $amount_usd = $request["amount_paid"] / $kurs;
            } else {
                $amount_usd = $request["amount_paid"];
                $amount_idr = $request["amount_paid"] * $kurs;
            }
