...
class Capex_Closing_Payment_Model extends MY_Model
{
    ...
    public function getSelectedColumns()
    {
        return array(
            ...
            'tb_capex_purchase_requisitions.pr_date'                                => 'Pr Date',
            'tb_capex_purchase_requisitions.closing_date'                           => 'Closing Date',
            'tb_capex_purchase_requisitions.account'                                => 'Account',
            'SUM(tb_capex_purchase_requisition_details.total) as total_capex'       => 'Total',
            'tb_capex_purchase_requisitions.closing_notes'                          => 'Notes',
        );
    }
    ...
    public function getGroupedColumns()
    {
        return array(
            ...
            'tb_capex_purchase_requisitions.closing_date',
            'tb_capex_purchase_requisitions.account',
            // 'tb_capex_purchase_requisition_details.total',
            'tb_capex_purchase_requisitions.closing_notes',
    ...
    public function getSearchableColumns()
    {
        return array(
            ...
            // 'tb_capex_purchase_requisition_detail.total',
            'tb_capex_purchase_requisitions.closing_notes',
            'tb_capex_purchase_requisitions.status',
            'tb_departments.department_name'
        );
    ...
    private function searchIndex()
    {
        if (!empty($_POST['columns'][1]['search']['value'])){
            ...
            $this->connection->where('tb_capex_purchase_requisitions.closing_date >= ', $range_date[0]);
            $this->connection->where('tb_capex_purchase_requisitions.closing_date <= ', $range_date[1]);
        }
    ...
    function getIndex($return = 'array')
    {
        ...
        $this->connection->join('tb_annual_cost_centers', 'tb_annual_cost_centers.id = tb_capex_monthly_budgets.annual_cost_center_id');
        ...
        $this->connection->where('tb_capex_purchase_requisitions.status', 'close');
    ...
    {
        $id                 = (isset($_SESSION['capex_closing']['id'])) ? $_SESSION['capex_closing']['id'] : NULL;
        $closing_date       = $_SESSION['capex_closing']['date'];
        $closing_by         = config_item('auth_person_name');
        $notes              = (empty($_SESSION['capex_closing']['closing_notes'])) ? NULL : $_SESSION['capex_closing']['closing_notes'];
        $account            = $_SESSION['capex_closing']['account'];
        ...
        $this->connection->select('tb_capex_purchase_requisition_details.quantity, tb_capex_purchase_requisition_details.id');
        $this->connection->from('tb_capex_purchase_requisition_details');
        $this->connection->where('tb_capex_purchase_requisition_details.capex_purchase_requisition_id', $id);
        ...
        foreach ($result as $data) {
            $this->connection->set('process_qty', '"process_qty" + ' . $data['quantity'], false);
