    ...
    case 'error':
        ...
        switch ($data['group'])
        {
            case 'url':
                ...
                {
                    case 'internal':
                        $message = 'Gagal menyambung ke host tertentu.'
                                 . 'Kemungkinan masalah besar bahwa server tidak ditemukan, koneksi time
out, atau koneksi ditolak oleh tuan rumah.'
                                 . 'Coba hubungkan lagi dan periksa apakah alamat sudah benar.';
                        break;
                    case 'external':
                        switch ($data['error'])
                        {
                            case 1:
                                $message = 'Anda mencoba untuk mengakses blacklist oleh server ini. Silakan pilih URL lain.';
                                break;
                            case 2:
                                $message = 'URL yang anda masukkan
salah. Silakan periksa apakah anda memasukkan URL yang benar atau tidak.';
                                break;
                        }
                        break;
                }
            ...
            case 'resource':
                ...
                switch ($data['type'])
                {
                    case 'file_size':
                        ...
                        break;
                    case 'hotlinking':
                        $message = 'Tampaknya anda mencoba untuk mengakses sumber daya melalui proxy ini dari website terpencil.<br />'
                                 . 'Untuk alasan keamanan, silahkan isi formulir di bawah ini untuk melakukannya.';
                        break;
                }
                break;
        }
        
        echo 'Sebuah kesalahan telah terjadi ketika mencoba untuk menelusuri proxy <br />' . $message . '</p></div>';
        break;
}
?>
  <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
    <ul id="form">
      Enter the URL<input id="address_box" type="text" name="<?php echo $GLOBALS['_config']['url_var_name'] ?>" value="<?php echo isset($GLOBALS['_url']) ? htmlspecialchars($GLOBALS['_url']) : '' ?>" onfocus="this.select()" /> <input id="go" type="submit" value="Go!!!" /></form>
<br/>&copy;2015 DarkIT
</div>
</body>
</html>
