            ...
            <table class="table table-hover" id="table-document">
              ...
              <tbody>
                <?php foreach ($_SESSION['request_closing']['items'] as $i => $items) : ?>
                  <?php $grand_total[] = $items['total']-$items['process_amount']; ?>
                  <tr id="row_<?= $i; ?>">
                    <td>
                  ...
                  <?php foreach ($items['request_detail'] as $j => $detail) : ?>                    
                    <tr class="detail_<?= $i; ?> hide">
                      ...
                      <td class="" colspan="2">
                        ...
                        <input name="account_code[]" id="account_code_<?= $i ?>_<?= $j ?>" data-parent="<?= $i ?>" data-row="<?= $j ?>" type="hidden" class="form-control-payment" value="<?=$detail['product_code']?>">
                        <?= print_string($detail['product_code']); ?> - <?= print_string($detail['product_name']); ?>
                      </td>
                      <td>
