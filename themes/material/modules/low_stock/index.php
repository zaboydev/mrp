...
<?php startblock('actions_right') ?>
  <?php if (is_granted($module, 'document')):?>
    <div class="section-floating-action-row">
      <button type="button" data-source = "<?=site_url('Purchase_Request'.'/create_item_purchase/');?>"  class="btn btn-floating-action btn-lg btn-primary btn-tooltip ink-reaction" id="modal-approve-data-button-multi">
          <i class="md md-assignment"></i>
          <small class="top right">Create Request</small>
        </button>
...
<?php startblock('datafilter') ?>
  <div class="form force-padding">
    ...
    <div class="form-group" style="margin-top: 40px">
      ...
      <input class="form-control input-sm filter_daterange" data-column="3" id="filter_required_date" readonly>
    </div>
    ...
    <div class="form-group">
      ...
      <select class="form-control input-sm filter_dropdown" data-column="8" id="filter_item_category">
        <option value="">
          Not filtered
        </option>
...
<?php startblock('scripts')?>
  ...
  $('input[type=radio][name=request_to]').change(function() {
    ...
    console.log(val);
    $.get( url, { data: val },function(data){
      ...
      if(id_purchase_order !== ""){
             $.post( action, {'id_purchase_order':id_purchase_order} ).done(function(data){
                ...
                if ( result.success == false ){
                  ...
                  window.location.href = '<?=site_url('Purchase_Request'.'/create_purchase');?>';
                }
              }).fail(function(){
                $("#modal-approve-data-button-multi").attr('disabled', false);
                toastr.options.timeOut = 10000;
      ...
      if(e.target.nodeName ==="INPUT"){
        if($(e.target).attr("type")==="checkbox"){
          if($(e.target).prop('checked')){
            ...
            on_hand_qty +="|"+$(e.target).attr('data-qty');
          }else {
            id_purchase_order = id_purchase_order.replace("|"+$(e.target).attr('data-id')+",","");
            on_hand_qty = on_hand_qty.replace("|"+$(e.target).attr('data-qty'),"");
          }
        }
          console.log(id_purchase_order);
          console.log(on_hand_qty);      
      }
