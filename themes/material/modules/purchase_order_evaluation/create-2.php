          ...
          <div class="row">
            ...
            <div class="col-sm-6 col-lg-3">
              <div class="form-group">
                ...
                <input type="hidden" name="head_dept" id="head_dept" class="form-control" value="<?= $_SESSION['poe']['head_dept']; ?>" data-input-type="autoset" data-source="<?= site_url($module['route'] . '/set_head_dept'); ?>">
              </div>
            </div>
          </div>
          <div class="row">
              ...
              <tbody>
                <?php foreach ($_SESSION['poe']['request'] as $id => $request) : ?>
                  <tr id="row_<?= $id; ?>">
                    ...
                    <?php $price = array(); ?>
                    <?php foreach ($_SESSION['poe']['vendors'] as $key => $vendor) : ?>
                      <?php $price[] = $_SESSION['poe']['request'][$id]['vendors'][$key]['total']; ?>
                    <?php endforeach; ?>
                    <?php $min = min($price); ?>
                    <?php foreach ($_SESSION['poe']['vendors'] as $key => $vendor) : ?>
                      <?php
                            if ($min == $_SESSION['poe']['request'][$id]['vendors'][$key]['total']) {
                      ...
                      <td style="<?= $style; ?>">
                        <?= anchor($module['route'] . '/set_selected_vendor/' . $id . '/' . $key, number_format($_SESSION['poe']['request'][$id]['vendors'][$key]['unit_price'], 2), 'style="color: black"'); ?>
