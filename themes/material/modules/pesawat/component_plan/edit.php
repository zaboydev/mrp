    ...
    <div class="card-body">
        <div class="row">
            <div class="col-sm-12 col-lg-8">
                <div class="form-group" style="padding-top: 20px;">
                    <select name="aircraft_register" id="aircraft_register" data-tag-name="aircraft_register" class="form-control input-sm select2" style="width: 100%" required>
                        ...
                        <option value="<?= $pesawat; ?>" <?= ($entity['aircraft_register'] == $pesawat) ? 'selected' : ''; ?>><?= $pesawat; ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="form-group">
                    <input type="date" name="date" id="date" class="form-control" value="<?= $entity['date']?>">
                    <label for="date">Date</label>
                </div>

                <div class="form-group">
                    <input type="text" name="year_plan" id="year_plan" class="form-control" maxlength="4" pattern="[0-9]{4}" value="<?= $entity['year_plan']?>">
                    <label for="date">Year Plan</label>
                </div>

                <div class="form-group">
                    <input type="text" name="description" id="description" class="form-control" value="<?= $entity['description']?>" required>
                    <label for="description">Description</label>
                </div>

                <div class="form-group">
                    <input type="text" name="part_number" id="part_number" class="form-control input-sm input-autocomplete" data-source="<?= site_url($module['route'] . '/search_items_by_part_number/'); ?>" value="<?= $entity['part_number']?>" required>
                ...
                <div class="form-group">
                    ...
                    <?php foreach (available_units() as $unit) : ?>
                        <option value="<?= $unit; ?>" <?= ($entity['unit'] == $unit) ? 'selected' : ''; ?>>
                        <?= $unit; ?>
                        </option>
                    <?php endforeach; ?>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <?php foreach (available_item_groups(config_item('auth_inventory')) as $i => $group) : ?>
                        <option value="<?= $group; ?>"<?= ($entity['group'] == $group) ? 'selected' : ''; ?>>
                        <?= $group; ?>
                        </option>
                    <?php endforeach; ?>
                    </select>
            ...
            <div class="col-sm-12 col-lg-4">
                <div class="form-group">
                    <input type="number" name="planing_quantity" id="planing_quantity" class="form-control" value="<?= $entity['planing_quantity']?>">
                    <label for="planing_quantity">Planning Quantity</label>
                </div>

                <div class="form-group">
                    <textarea name="remarks" id="remarks" class="form-control" rows="4"><?= $entity['remarks']?></textarea>
