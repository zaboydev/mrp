            ...
            <tbody>
              <?php foreach ($_SESSION['expense_poe']['request'] as $id => $request) : ?>
                <tr id="row_<?= $id; ?>">
                  ...
                  <td class="hide">
                    <input type="number" rel="konversi" name="request[<?= $id; ?>][konversi]" value="<?= $_SESSION['expense_poe']['request'][$id]['konversi']; ?>" class="form-control">
                  </td>
                  <td>
                    <input type="text" rel="remarks" name="request[<?= $id; ?>][remarks]" value="<?= $_SESSION['expense_poe']['request'][$id]['remarks']; ?>" class="form-control">
                  </td>

                  <?php foreach ($_SESSION['expense_poe']['vendors'] as $key => $vendor) : ?>
                    <td>
                      <input type="text" rel="unit_price" name="request[<?= $id; ?>][vendors][<?= $key; ?>][unit_price]" value="<?= $_SESSION['expense_poe']['request'][$id]['vendors'][$key]['unit_price']; ?>" class="form-control">
                    </td>

                    <td>
                      <input type="text" rel="core_charge" name="request[<?= $id; ?>][vendors][<?= $key; ?>][core_charge]" value="<?= $_SESSION['expense_poe']['request'][$id]['vendors'][$key]['core_charge']; ?>" class="form-control">
                    </td>
                  <?php endforeach; ?>
                </tr>
              <?php endforeach; ?>
