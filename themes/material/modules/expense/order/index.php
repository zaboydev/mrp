...
<script>
  ...
  $(function() {
    ...
    <?php
    if (is_granted($modules['capex_purchase_order'], 'approval')) {
      ?>
    ...
    function getAttachment(id) {
      $.ajax({
        type: "GET",
        url: 'expense_purchase_order/listAttachment/' + id,
        cache: false,
        success: function(response) {
          var data = jQuery.parseJSON(response)
          $("#listView").html("")
          ...
          if(data.count_att_po>0){
            ...
            var no_att_po = '<tr><td colspan="2" style="text-align:center;">PO doesnt have Attachment</td></tr>';
    ...
    $("#modal-reject-data-button-multi").click(function() {
      if (!encodeNotes()) {
        ...
        if (id_purchase_order == "") {
          ...
          $.ajax({
            type: "POST",
            url: 'expense_purchase_order/multi_reject',
            data: {
              "id_purchase_order": id_purchase_order,
              "notes": notes
            },
