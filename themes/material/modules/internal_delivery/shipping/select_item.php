          ...
          <table class="table table-hover" id="tbl_prl">
            <thead>
              <tr>
                ...
                <th>Received From</th>
                <th>Received Date</th>
                <?php if($_SESSION['return']['source']=='stock'):?>
                <th>Expired Date</th>
                <?php endif;?>
              </tr>
            </thead>
              ...
              <?php foreach ($entities as $e => $entity) : ?>
                <tr>
                    <td>
                        ...
                        <input type="checkbox" name="item_id[]" id="item_id_<?= $e; ?>" value="<?= $entity['id']; ?>" <?= (in_array($entity['id'], array_keys($_SESSION['shipping_internal']['items']))) ? 'checked' : ''; ?>>
                        <label for="item_id_<?= $e; ?>">
                            <?= $e + 1; ?>
                        </label>
                        </div>
                    </td>
                    ...
                    <td>
                        <label for="item_id_<?= $e; ?>">
                        <?= print_string($entity['received_from']); ?>
                        </label>
                    </td>
                    <td>
                        <label for="item_id_<?= $e; ?>">
