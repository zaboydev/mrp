...
class Warehouse extends MY_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['warehouse'];
  ...
  public function index()
  {
    ...
    $this->data['page']['title']            = 'Warehouse';
  ...
  public function save()
  {
    ...
    if (is_granted($this->module, 'save') === FALSE){
      ...
      if ($this->input->post('id')){
        if ($this->model->isWarehouseExists($this->input->post('warehouse'), NULL, $this->input->post('warehouse_exception'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Warehouse! Warehouse '. $this->input->post('warehouse') .' already exists.';
        } else {
          if ($this->model->update($this->input->post('id'))){
            $return['type'] = 'success';
            $return['info'] = 'Warehouse ' . $this->input->post('warehouse') .' updated.';
          } else {
            $return['type'] = 'danger';
        ...
        if ($this->model->isWarehouseExists($this->input->post('warehouse'))){
          $return['type'] = 'danger';
          $return['info'] = 'Duplicate Warehouse! Warehouse '. $this->input->post('warehouse') .' already exists.';
        } else {
          if ($this->model->insert()){
            $return['type'] = 'success';
            $return['info'] = 'Warehouse ' . $this->input->post('warehouse') .' created.';
          } else {
            $return['type'] = 'danger';
            $return['info'] = 'There are error while updating data. Please try again later.';
          }
