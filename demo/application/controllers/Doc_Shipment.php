...
class Doc_Shipment extends MY_Controller
{
  protected $module;
  ...
  public function __construct()
  {
    ...
    $this->module = $this->modules['doc_shipment'];
  ...
  public function set_document_sent_by()
  {
    if ($this->input->is_ajax_request() == FALSE)
      redirect($this->modules['secure']['route'] . '/denied');
  ...
  public function save()
  {
    ...
    if (is_granted($this->module, 'save') == FALSE){
      ...
      if (!isset($_SESSION['shipment']['items']) || empty($_SESSION['shipment']['items'])){
        ...
        if (isset($_SESSION['shipment']['edit']) && $_SESSION['shipment']['edit'] !== $document_number){
          if ( $this->model->isDocumentNumberExists($document_number)){
            $data['success'] = FALSE;
            $data['message'] = 'Duplicate Document Number: '. $_SESSION['shipment']['document_number'] .' ! Try using '. shipment_last_number() .' for document number.';
          }
        }
  ...
  public function add_item($id = NULL)
  {
    ...
    if (isset($_POST) && !empty($_POST)){
      if (!isset($_SESSION['shipment'])){
        $_SESSION['shipment']['items']            = array();
        $_SESSION['shipment']['category']    	  = available_categories($this->input->post('group'));
        $_SESSION['shipment']['document_number']  = shipment_last_number();
        $_SESSION['shipment']['issued_date']      = date('Y-m-d');
        $_SESSION['shipment']['issued_by']        = NULL;
        $_SESSION['shipment']['issued_to']        = NULL;
